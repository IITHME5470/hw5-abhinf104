...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob,
                        double *x, double *y, double **T, double *xleftghost, double *xrightghost,
                        double *sendbuf_x, double *recvbuf_x)
{
  MPI_Status status;

  // Determine neighboring ranks in x-direction
  int left_nb = (rank_x == 0) ? MPI_PROC_NULL : rank - 1;
  int right_nb = (rank_x == px - 1) ? MPI_PROC_NULL : rank + 1;
  int i, j;
  // Exchange in x-direction: send to left, receive from right
  // Pack send buffer with leftmost column data
  for (j = 0; j < ny; j++)
  {
    sendbuf_x[j] = T[0][j];
  }
  ...
  // Unpack received data into right ghost cells
  for (j = 0; j < ny; j++)
  {
    xrightghost[j] = recvbuf_x[j];
  }

  // Exchange in x-direction: send to right, receive from left
  // Pack send buffer with rightmost column data
  for (j = 0; j < ny; j++)
  {
  ...
  // Unpack received data into left ghost cells
  for (j = 0; j < ny; j++)
  {
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  MPI_Sendrecv(sendbuf_y, nx, MPI_DOUBLE, bot_nb, 0,
               recvbuf_y, nx, MPI_DOUBLE, top_nb, 0,
  ...
  MPI_Sendrecv(sendbuf_y, nx, MPI_DOUBLE, top_nb, 0,
               recvbuf_y, nx, MPI_DOUBLE, bot_nb, 0,
               MPI_COMM_WORLD, &status);
